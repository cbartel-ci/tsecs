...
test('integration test', () => {
  ...
  const world: World = new WorldBuilder()
    .with(entitySystemA, entitySystemB, entitySystemC)
    .build();

  const componentAMapper = world.getComponentMapper<TestComponentA>(
    TestComponentA
  );
  const componentBMapper = world.getComponentMapper<TestComponentB>(
    TestComponentB
  );
  const componentCMapper = world.getComponentMapper<TestComponentC>(
    TestComponentC
  );
  const componentDMapper = world.getComponentMapper<TestComponentD>(
    TestComponentD
  );
  ...
  world.update(0);
  expect(entitySystemA.getEntities()).toStrictEqual([
    entity1,
    entity2,
    entity4,
  ]);
  ...
  expect(entitySystemC.getEntities()).toStrictEqual([
    entity1,
    entity2,
    entity3,
  ]);
